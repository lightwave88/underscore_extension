...
const $tagNameSolutionList = require('./method_2a');
...
class FindTags {
            ...
            if (tagStartIndex != null) {
                ...
                if (attrSymbol == null) {
                    ...
                    if (/>/.test(_chart)) {
                        ...
                        node = this._findSpecTagSolution(tagContent);
                ...
                if (!/</.test(_chart)){
                    ...
                    if ((res = $Tools.isCommandTagHead(rightContent)) != null) {
                        console.log('有 command tag');
                        tagName = res[1];
                        res = this._findTagSolution_by_tagName(i, content, tagName, false);
                        index = res.index;
                        node = res.node;
                        ...
                        res = this._findTagSolution_by_tagName(i, content, tagName, isEndtag);

                        index = res.index;
                        node = res.node;

                        console.log('tagName: %s', node.tagName);
    ...
    // 若已經確定 nodeName
    // i: 絕對座標
    _findTagSolution_by_tagName(i, content, tagName, endTag) {
        // debugger;

        console.log('_findTagSolution_by_tagName()>');
        ...
        if (endTag) {
            ...
            if (solution != null) {
                ...
                console.log('根據 tagName(%s) 找到特殊解(%s)', tagName, res.node.tagName);
    ...
    // 找尋 tag 轉屬的模組
    // 找出他的結尾與結尾的 index
    _findSpecTagSolution(tagContent) {
